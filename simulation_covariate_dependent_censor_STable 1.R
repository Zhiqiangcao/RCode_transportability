...
##under covariate-dependent censoring for weak and strong sampling 
...
setwd("C:/Users/82655/Dropbox/research/generability/R_code_generability_bai/submission")
N <- 1000000
...
if(scenario == 1){ #ccc model   
  cindex = c(1,3); sindex = c(2,3); condtype = "cox";
}else if(scenario == 2){ #wwc model
  cindex = c(3); sindex = c(3); condtype = "cox";
}else if(scenario == 3){ #ccw model
  cindex = c(1,3); sindex = c(1,3); condtype = "lognorm";
}else if(scenario == 4){ #wcc model
  cindex = c(3); sindex = c(2,3); condtype = "cox";
}else if(scenario ==5){  #cwc model
  cindex = c(1,3); sindex = c(3); condtype = "cox";
}else{  #www model
  cindex = c(3); sindex = c(3); condtype = "lognorm";
}
...
choice = 1					
if(choice==1){						
  gampar <- c(-6.5,-0.1,-0.5)   #weak sampling						
  true_val <- c(-0.07714072,-0.14101986,-0.17789355)  
  
}else{						
  # selection probability for trial						
  gampar <- c(-6.7,-0.4,-1)  #strong sampling						
  true_val <- c(-0.07711237,-0.14096867,-0.17782954) #true value						
}

###main simulation
for(v in 1:nsim){
  set.seed(1000+v)  #original set.seed 1000+v
  # Generate covariates
  x1 <- rnorm(N)
  x2 <- rnorm(N)
  x3 <- rbinom(N,1,0.5)
  ...
  xsc <- cbind(x1,x3)
  p_selS <- samp(gampar,cbind(1,xsc))
  S <- rbinom(N,1,p_selS)
  # population data
...
resu_final
mean(cen.rate)
write.csv(resu_final,paste0('cova_dependent_scen',scenario,'_diff_cova_cho1','.csv'))
